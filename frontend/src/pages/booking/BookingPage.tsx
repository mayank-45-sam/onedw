...
import { InstantBookingButton } from '@/components/booking/InstantBookingButton';
...
export default function BookingPage() {
  ...
  // ?service=<id> → auto-select the service and skip step 1
  useEffect(() => {
    if (!serviceParam || !servicesQuery.data?.data?.length || selectedService) return;
    const match = servicesQuery.data.data.find((s) => s._id === serviceParam);
    if (match) {
      setSelectedService(match);
      setStep((s) => Math.max(s, 2));
    }
  }, [serviceParam, servicesQuery.data?.data, selectedService]);
  ...
  // ?worker=<id> → auto-select the worker and skip steps 2-4
  useEffect(() => {
    if (!workerParam || !selectedService || !workersQuery.data?.data?.length || selectedWorker) return;
    const match = workersQuery.data.data.find((w) => w._id === workerParam);
    if (match) {
      setSelectedWorker(match);
      setStep((s) => Math.max(s, 5));
    }
  }, [workerParam, selectedService, workersQuery.data?.data, selectedWorker]);
  ...
  const handleBidSuccess = () => {
    toast.success('Bid request sent successfully! View it in your marketplace.');
    setBidFormOpen(false);
    navigate(ROUTES.biddingHome);
  };
         ...
         defaultValues={
           selectedService && problem
             ? {
                 ...
                 description: problem,
                 budgetMin: Math.round(selectedService.basePrice * 0.8),
                 budgetMax: Math.round(selectedService.basePrice * 1.2),
                 urgency: 'asap',
               }
             : undefined
         }
