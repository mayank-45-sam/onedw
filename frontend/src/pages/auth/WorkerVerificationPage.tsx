...
export default function WorkerVerificationPage() {
  ...
  const [testLanguage, setTestLanguage] = useState<'en' | 'tamil' | 'hindi'>('en');
  ...
  // ── Skill test ──────────────────────────────────────────────────
  useEffect(() => {
    if (stage === 'skill_test' && languagePicked && questions.length === 0 && !generating) {
      setGenerating(true);
      verificationService
        .generateSkillTest(testLanguage)
        .then((res) => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stage, languagePicked, testLanguage, questions.length, generating]);
          ...
          >
            ...
            {stage === 'skill_test' && (
              <div className="card-premium overflow-hidden">
                ...
                ) : questions.length === 0 && !languagePicked ? (
                  <SkillTestLanguagePicker
                    selected={testLanguage}
                    onSelect={setTestLanguage}
                    onStart={() => setLanguagePicked(true)}
                  />
                ) : questions.length === 0 ? (
...
const TEST_LANGUAGES: { id: 'en' | 'tamil' | 'hindi'; label: string; native: string }[] = [
  { id: 'en', label: 'English', native: 'English' },
  { id: 'tamil', label: 'Tamil', native: 'தமிழ்' },
  { id: 'hindi', label: 'Hindi', native: 'हिन्दी' },
];

function SkillTestLanguagePicker({
  selected,
  onSelect,
  onStart,
}: {
  selected: 'en' | 'tamil' | 'hindi';
  onSelect: (lang: 'en' | 'tamil' | 'hindi') => void;
  onStart: () => void;
}) {
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-lg font-bold">Choose your test language</h2>
          <p className="text-xs text-muted-foreground">
        ...
        {TEST_LANGUAGES.map((lang) => (
          <button
            ...
            className={cn(
              ...
              selected === lang.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'
            )}
          >
            <span className="block text-sm font-semibold">{lang.label}</span>
            <span className="mt-1 block text-lg font-bold font-display text-primary">{lang.native}</span>
            {selected === lang.id && <CheckCircle2 className="mt-2 h-5 w-5 text-primary" />}
          </button>
        ))}
      </div>

      <Button className="btn-glow mt-6 w-full gap-2 rounded-xl" onClick={onStart}>
        <Sparkles className="h-4 w-4" /> Start skill test
      </Button>
    </div>
  );
}
