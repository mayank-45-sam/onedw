...
export default function SearchPage() {
  ...
  const serviceSort =
    sort === 'budget' ? ('price-asc' as const) : sort === 'rating' || sort === 'popular' ? (sort as 'rating' | 'popular') : undefined;
  const workerSort = sort === 'budget' ? ('price' as const) : sort === 'rating' ? ('rating' as const) : undefined;

  const servicesQuery = useQuery({
    queryKey: queryKeys.services.all({ search: q, limit: 12, sort: serviceSort }),
    queryFn: () => serviceService.list({ search: q, limit: 12, sort: serviceSort }),
    enabled: q.length > 0,
  });
  const workersQuery = useQuery({
    queryKey: queryKeys.workers.all({ search: q, limit: 8, sort: workerSort }),
    queryFn: () => workerService.list({ search: q, limit: 8, sort: workerSort }),
    enabled: q.length > 0,
  });
  ...
  const budgetWorker = aiSearchQuery.data?.budgetWorker
    ?? deriveFallbackWorker(allWorkers, (a, b) => a.hourlyRate - b.hourlyRate);

  const fastestWorker = aiSearchQuery.data?.fastestWorker
    ?? deriveFallbackWorker(allWorkers, (a, b) => (b.completedJobs ?? 0) - (a.completedJobs ?? 0));

  const highestRatedWorker = aiSearchQuery.data?.highestRatedWorker
    ?? deriveFallbackWorker(allWorkers, (a, b) => (b.rating ?? 0) - (a.rating ?? 0));
              ...
              <div className="flex flex-wrap gap-2">
                {recommendedFilters.map((f) => (
                  <button
                    ...
                    onClick={() => {
                      const next = new URLSearchParams(params);
                      next.set('sort', f.value);
                      setParams(next, { replace: true });
                    }}
                    className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs transition hover:border-primary hover:bg-primary/5 ${
                      sort === f.value ? 'border-primary bg-primary/10 text-primary' : 'bg-card'
                    }`}
                  >
                    {f.label}
