...
import { WorkerBookingActions } from '@/components/cards/WorkerBookingActions';
...
import { advanceBooking } from '@/utils/bookingActions';
import type { BookingStatus, Worker } from '@/types';
...
export default function WorkerDashboardPage() {
  ...
  const progressMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: BookingStatus }) => advanceBooking(id, status),
    onSuccess: () => {
      toast.success('Booking updated');
      invalidateBookings();
    },
    onError: (err) => toast.error(err instanceof ApiError ? err.message : 'Could not update booking'),
  });
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button asChild variant="outline" className="gap-2 rounded-full">
            <Link to={ROUTES.biddingHome}><Wrench className="h-4 w-4" /> Browse custom jobs</Link>
          </Button>
        </div>
      </div>
        ...
        <div>
          ...
          {upcoming.isLoading ? (
            ...
            <div className="space-y-4">
              {upcomingJobs.slice(0, 5).map((b, i) => (
                <div key={b._id} className="space-y-3">
                  <BookingCard booking={b} index={i} />
                  <WorkerBookingActions
                    booking={b}
                    onAction={(status, booking) =>
                      progressMutation.mutate({ id: booking._id, status })
                    }
                    busy={progressMutation.isPending && progressMutation.variables?.id === b._id}
                  />
                </div>
              ))}
            </div>
