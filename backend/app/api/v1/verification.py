"""Verification API — async Beanie version."""
from __future__ import annotations

from datetime import datetime
from typing import Optional

from fastapi import APIRouter, Depends, File, HTTPException, UploadFile
from pydantic import Field

from app.dependencies.auth import RequireWorker, get_current_user
from app.models.user import User
from app.models.worker import Worker
from app.models.certificate import Certificate
from app.models.portfolio_image import PortfolioImage
from app.schemas.common import SchemaBase
from app.schemas.verification import (
    SkillTestSubmitRequest,
    PracticalSubmitRequest,
    InterviewRespondRequest,
    DocumentsSubmitRequest,
)
from app.core.exceptions import BadRequestException, NotFoundException
from app.services import verification_service as vs

router = APIRouter(prefix="/verification", tags=["Worker Verification"])


class StartVerificationRequest(SchemaBase):
    profession: Optional[str] = Field(None, max_length=255)


class SkillTestGenerateRequest(SchemaBase):
    language: Optional[str] = Field(None, max_length=20)


async def _get_worker(user: User) -> Worker:
    worker = await Worker.find_one(Worker.user_id == user.id)
    if worker is None:
        raise BadRequestException(message="Worker profile not found")
    return worker


async def _get_in_progress_verification(worker: Worker):
    verification = await vs.get_active_verification(worker.id)
    if verification is None:
        raise BadRequestException(message="No verification in progress. Start verification first.")
    return verification


@router.get("/status", summary="Get current verification status")
async def get_verification_status(current_user: User = Depends(RequireWorker)):
    worker = await _get_worker(current_user)
    latest = await vs.get_latest_verification(worker.id)
    active = await vs.get_active_verification(worker.id)

    profile = {
        "name": worker.name, "profession": worker.profession,
        "experience_years": worker.experience_years,
        "aadhaar_verified": getattr(worker, "aadhaar_verified", False),
        "certificates_count": len(getattr(worker, "certificates", []) or []),
        "portfolio_count": len(getattr(worker, "portfolio_images", []) or []),
    }
    return {
        "success": True, "message": "OK",
        "data": {
            "worker": profile,
            "has_active": active is not None,
            "active": await vs.serialize_verification(active) if active else None,
            "latest": await vs.serialize_verification(latest) if latest else None,
            "retry_available_at": (
                latest.retry_available_at.isoformat()
                if latest and latest.retry_available_at and latest.badge == vs.BADGE_REJECTED
                else None
            ),
        },
    }


@router.post("/start", summary="Start a new verification attempt")
async def start_verification(
    body: StartVerificationRequest,
    current_user: User = Depends(RequireWorker),
):
    worker = await _get_worker(current_user)
    active = await vs.get_active_verification(worker.id)
    if active:
        return {"success": True, "message": "Verification already in progress",
                "data": await vs.serialize_verification(active)}

    if body.profession:
        worker.profession = body.profession
        await worker.save()

    verification = await vs.create_verification(worker)
    worker.verification_status = "in_progress"
    await worker.save()

    return {"success": True, "message": "Verification started",
            "data": await vs.serialize_verification(verification)}


@router.post("/documents", summary="Submit profile details and identity documents")
async def submit_documents(
    body: DocumentsSubmitRequest,
    current_user: User = Depends(RequireWorker),
):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)

    from app.core.security import hash_token

    if body.profession and body.profession.strip():
        worker.profession = body.profession.strip()
        verification.profession = worker.profession
    if body.experience_years is not None:
        worker.experience_years = body.experience_years
    if body.aadhaar_number:
        worker.aadhaar_number_hash = hash_token(body.aadhaar_number)
        worker.aadhaar_verified = False

    # Add portfolio images
    existing_portfolios = await PortfolioImage.find(PortfolioImage.worker_id == worker.id).to_list()
    existing_urls = {p.url for p in existing_portfolios}
    for url in (body.work_photos or []):
        if url and url not in existing_urls:
            await PortfolioImage(worker_id=worker.id, url=url).insert()
            existing_urls.add(url)

    # Add certificates
    if body.certificate_images:
        existing_certs = await Certificate.find(Certificate.worker_id == worker.id).to_list()
        existing_cert_images = {c.image for c in existing_certs}
        for i, url in enumerate(body.certificate_images):
            if url and url not in existing_cert_images:
                await Certificate(
                    worker_id=worker.id,
                    title=f"Verification Certificate {i + 1}",
                    image=url,
                    issued_at=datetime.now(),
                ).insert()

    verification.document_media = {
        "certificate_images": list(dict.fromkeys(body.certificate_images or [])),
        "work_photos": list(dict.fromkeys(body.work_photos or [])),
        "work_videos": list(dict.fromkeys(body.work_videos or [])),
    }
    verification.step = "skill_test"
    await verification.save()
    await worker.save()

    return {
        "success": True, "message": "Profile and documents saved",
        "data": {
            "verification": await vs.serialize_verification(verification),
            "documents_score": await vs.compute_documents_score(worker),
            "experience_score": await vs.compute_experience_score(worker),
            "aadhaar_verified": getattr(worker, "aadhaar_verified", False),
        },
    }


@router.post("/skill-test/generate", summary="Generate AI technical test questions")
async def generate_skill_test(
    body: SkillTestGenerateRequest,
    current_user: User = Depends(RequireWorker),
):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)
    language = body.language if body.language in vs.SUPPORTED_TEST_LANGUAGES else "en"

    session = await vs.get_skill_test_session(verification.id)
    if session is None:
        session = await vs.create_skill_test_session(verification, worker, language)

    stored_language = None
    if session.questions:
        stored_language = (session.questions[0] or {}).get("language", "en")

    if not session.questions or stored_language != language:
        questions = vs.generate_skill_test_questions(verification.profession, language)
        session.questions = questions
        verification.step = "skill_test"
        await session.save()
        await verification.save()

    return {
        "success": True, "message": "Skill test generated",
        "data": {
            "session_id": session.id, "profession": session.profession,
            "language": language, "questions": vs.public_questions(session.questions or []),
            "tab_switch_count": session.tab_switch_count, "status": session.status,
        },
    }


@router.post("/skill-test/submit", summary="Submit skill test answers with anti-cheat analytics")
async def submit_skill_test(
    body: SkillTestSubmitRequest,
    current_user: User = Depends(RequireWorker),
):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)
    session = await vs.get_skill_test_session(verification.id)
    if session is None:
        raise BadRequestException(message="No skill test found. Generate the test first.")
    if session.status in ("submitted", "failed"):
        raise BadRequestException(message="Skill test already submitted.")

    answers = [a.model_dump() for a in body.answers]
    anti_cheat = body.anti_cheat.model_dump()
    result = vs.evaluate_skill_test(session, answers, anti_cheat)
    verification.technical_score = result["score"]
    verification.skill_test_anti_cheat = result["analytics"]

    if session.failed:
        verification.status = "failed"
        verification.retry_available_at = datetime.now()
        worker.verification_status = "rejected"
        await verification.save()
        await worker.save()
        return {
            "success": True, "message": "Skill test failed due to anti-cheating rules",
            "data": {"failed": True, "reason": "tab_switch_limit",
                     "score": 0.0, "analytics": result["analytics"]},
        }

    verification.step = "practical"
    await verification.save()
    return {
        "success": True, "message": "Skill test submitted",
        "data": {
            "score": result["score"], "correct_count": result["correct_count"],
            "total": result["total"], "analytics": result["analytics"], "next_step": "practical",
        },
    }


@router.post("/practical/submit", summary="Submit work media for AI practical assessment")
async def submit_practical(
    body: PracticalSubmitRequest,
    current_user: User = Depends(RequireWorker),
):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)
    if verification.technical_score is None:
        raise BadRequestException(message="Complete the technical test before the practical assessment.")
    media_urls = [m.model_dump() for m in body.media_urls]
    result = await vs.evaluate_practical(verification, media_urls)
    return {
        "success": True, "message": "Practical assessment submitted",
        "data": {"score": result["score"], "evaluation": result["evaluation"], "next_step": "interview"},
    }


@router.post("/interview/start", summary="Start the AI voice interview")
async def start_interview(current_user: User = Depends(RequireWorker)):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)
    if verification.practical_score is None:
        raise BadRequestException(message="Complete the practical assessment before the interview.")
    result = await vs.start_interview(verification)
    return {"success": True, "message": "Interview started", "data": result}


@router.post("/interview/respond", summary="Answer the interviewer; get the next question")
async def respond_interview(
    body: InterviewRespondRequest,
    current_user: User = Depends(RequireWorker),
):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)
    result = await vs.respond_interview(verification, body.answer, body.mode)
    return {"success": True, "message": "Interview progress saved", "data": result}


@router.post("/interview/transcribe", summary="Transcribe interview voice audio")
async def transcribe_interview_audio(
    file: UploadFile = File(...),
    current_user: User = Depends(RequireWorker),
):
    await _get_worker(current_user)
    data = await file.read()
    if not data:
        raise BadRequestException(message="Empty audio file")
    text = vs.transcribe_audio(data, file.content_type or "audio/webm")
    if not text:
        raise HTTPException(status_code=502, detail="Speech-to-text is temporarily unavailable. Please type your answer.")
    return {"success": True, "message": "Transcribed", "data": {"text": text}}


@router.post("/complete", summary="Finalise verification: trust score, badge, certificate")
async def complete_verification(current_user: User = Depends(RequireWorker)):
    worker = await _get_worker(current_user)
    verification = await _get_in_progress_verification(worker)

    if verification.technical_score is None:
        raise BadRequestException(message="Complete the technical test first.")
    if verification.practical_score is None:
        raise BadRequestException(message="Complete the practical assessment first.")
    if verification.interview_score is None:
        raise BadRequestException(message="Complete the interview first.")

    result = await vs.complete_verification(verification, worker)
    return {
        "success": True, "message": "Verification complete",
        "data": {
            **result,
            "worker": {
                "verification_status": worker.verification_status,
                "trust_score": worker.trust_score,
                "verification_badge": worker.verification_badge,
                "is_verified": False,
            },
        },
    }


@router.get("/certificate", summary="Get the current worker's verification certificate")
async def get_my_certificate(current_user: User = Depends(RequireWorker)):
    worker = await _get_worker(current_user)
    latest = await vs.get_latest_verification(worker.id)
    if latest is None or latest.badge in (None, vs.BADGE_REJECTED):
        raise NotFoundException(message="No certificate issued yet")

    from app.models.certificate import Certificate as CertModel
    cert = await CertModel.find_one(CertModel.worker_id == worker.id)
    if cert is None:
        raise NotFoundException(message="No certificate issued yet")

    return {
        "success": True, "message": "OK",
        "data": {
            "certificate_no": getattr(cert, "certificate_no", cert.id),
            "worker_name": getattr(cert, "worker_name", worker.name),
            "profession": getattr(cert, "profession", worker.profession),
            "trust_score": getattr(cert, "trust_score", worker.trust_score),
            "badge": getattr(cert, "badge", worker.verification_badge),
            "issued_at": cert.issued_at.isoformat() if cert.issued_at else None,
            "qr_code_url": getattr(cert, "qr_code_url", None),
            "pdf_url": getattr(cert, "pdf_url", None),
            "is_active": getattr(cert, "is_active", True),
        },
    }


@router.get("/verify/{certificate_no}", summary="Public certificate verification")
async def verify_certificate(certificate_no: str):
    from app.models.certificate import Certificate as CertModel
    cert = await CertModel.find_one(CertModel.certificate_no == certificate_no)
    if cert is None:
        raise NotFoundException(message="Certificate not found")
    return {
        "success": True, "message": "OK",
        "data": {
            "valid": getattr(cert, "is_active", True),
            "certificate_no": getattr(cert, "certificate_no", cert.id),
            "worker_name": getattr(cert, "worker_name", ""),
            "profession": getattr(cert, "profession", ""),
            "trust_score": getattr(cert, "trust_score", None),
            "badge": getattr(cert, "badge", None),
            "issued_at": cert.issued_at.isoformat() if cert.issued_at else None,
        },
    }
